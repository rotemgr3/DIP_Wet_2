...
class SIDD_Dataset(Dataset):
    ...
    def _get_block(self, instance_dir, block_idx):
        ...
        noisy_path = next(
            (os.path.join(instance_dir, f) for f in os.listdir(instance_dir) if f.startswith("NOISY_")), None
        )
        clean_path = next(
            (os.path.join(instance_dir, f) for f in os.listdir(instance_dir) if f.startswith("GT_")), None
        )
        ...
        # Convert to tensor
        return self._to_tensor_and_normalize(noisy_block), self._to_tensor_and_normalize(clean_block)
        ...
        for instance_idx, instance in enumerate(self.scene_instances):
            ...
            if noisy_path:
                ...
                if self.mode == 'test':
                    ...
                    index_map.extend([(instance_idx, block_idx) for block_idx in range(4)])
                else:
                    index_map.extend([(instance_idx, block_idx) for block_idx in range(num_blocks)])
        return index_map
