...
def train_model(config):
    ...
    for epoch in range(config["epochs"]):
        train_loss, train_psnr = train_one_epoch(
            model, train_dataloader, criterion, optimizer, manager.device
        )
        val_loss, val_psnr = evaluate_model(
            model, val_dataloader, criterion, manager.device
        )
    ...
    manager.save_metrics(
        {"train_psnr": train_psnr_list, "val_psnr": val_psnr_list},
        os.path.join(config["output_dir"], "metrics.json"),
    )
...
def test_model(config):
    ...
    criterion = nn.MSELoss()
    test_loss, test_psnr = evaluate_model(
        model, test_dataloader, criterion, manager.device
    )
