...
class UNetDenoiser(nn.Module):
    def __init__(self, input_shape, hidden_dims, kernel_size=3):
        ...
        for h_dim in hidden_dims:
            self.encoders.append(EncoderBlock(prev_channels, h_dim, kernel_size))
            prev_channels = h_dim

        # Bottleneck
        self.bottleneck = ResBlock(hidden_dims[-1], hidden_dims[-1] * 2, kernel_size)
        ...
        for h_dim in reversed_hidden_dims:
            self.decoders.append(DecoderBlock(prev_channels, h_dim, kernel_size))
            prev_channels = h_dim
